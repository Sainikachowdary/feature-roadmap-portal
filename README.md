...
python -m app.main
The FastAPI server runs at http://127.0.0.1:8000. Open API interactive docs at http://127.0.0.1:8000/docs.
...
Dual-Token Cookie Strategy: Used short-lived 15-minute access tokens alongside 7-day refresh tokens set in httpOnly cookies to protect against XSS and token hijacking.
Atomic Upvoting: Unique SQL constraints on (post_id, user_id) ensure that upvotes are toggled atomically without race conditions.
Optimistic UI Updates: The frontend immediately updates vote counts and active states on click, performing a silent rollback if the backend request fails.
